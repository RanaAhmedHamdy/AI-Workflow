...
Prerequisite: install [uv](https://docs.astral.sh/uv/getting-started/installation/) so `uvx --version` works. The public default branch has not yet published this release-candidate CLI, so evaluate the checked-out candidate rather than pretending an unpublished remote command works.
...
All remaining commands retain the tested `uvx --from . ai-workflow` prefix. Do not assume the first `uvx` invocation installs a global command. A release-pinned remote command belongs only after a candidate is actually published.
...
```bash
uvx --from . ai-workflow brownfield --platform android --profile safety --target /path/to/existing-android-app
uvx --from . ai-workflow route --workflow brownfield --platform android --profile safety --fact persistence --fact concurrency
```
...
uvx --from . ai-workflow route --workflow brownfield --platform ios --profile safety --fact persistence --fact concurrency --fact lifecycle
```
...
uvx --from . ai-workflow greenfield --platform ios --profile feature --target /path/to/new-ios-app
```
...
uvx --from . ai-workflow update --platform ios --target /path/to/mobile-repository --dry-run
```
...
```bash
uvx --from . ai-workflow profiles
uvx --from . ai-workflow status --target /path/to/mobile-repository
uvx --from . ai-workflow update --target /path/to/mobile-repository --profile feature --dry-run
uvx --from . ai-workflow uninstall --target /path/to/mobile-repository --dry-run
```
