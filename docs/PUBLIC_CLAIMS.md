...
| Provides Claude Code compatibility | Generated `CLAUDE.md` with `@AGENTS.md`/`@AI_CONTEXT.md` imports, `.claude/skills/` symlinks resolving to `.agents/skills/`, installer verified by executable run (macOS arm64, Python 3.12.7, ai-workflow 0.9.0); see `docs/verification/CLAUDE_CODE_COMPATIBILITY.md` | VERIFIED | "Generates a Claude Code project adapter (`CLAUDE.md`) that imports `AGENTS.md` and `AI_CONTEXT.md` via `@`-include directives, and installs namespaced `.claude/skills/` symlinks that resolve to the canonical `.agents/skills/` directories. Adapter structure, symlink resolution, and installer output verified by executable run. Live `/memory` and `/skills` CLI commands require a separate Claude Code CLI harness." |
