# Project-specific Graphify usage example

> **Example only.** This file preserves a real project-specific Graphify limitation/override record. Do not copy its revisions, paths, symbols, version-specific limitations, or conclusions into another repository as truth.
...
Absence of a graph edge is not evidence of absence. Agents must consult `docs/ai/examples/graphify-context-overrides.project-example.tsv` when working in scopes affected by these limitations and must verify dependency-sensitive claims against current source.
...
- the ViewModel constructor declarations under `app/src/main/java/com/stemcampathome/navigation/` and `app/src/main/java/com/stemcampathome/ui/screens/` listed row-by-row in `docs/ai/examples/graphify-context-overrides.project-example.tsv`;
