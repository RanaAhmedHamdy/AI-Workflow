...
Install repository governance and routing
        ↓
Create/refresh Architecture Spine
        ↓
Propose required ADRs
        ↓
Repository owner reviews ADRs
        ↓
Synchronize accepted architecture
        ↓
Cross-feature architecture coverage + readiness state
        ↓
Repository is ready for bounded feature intake
```
...
Use `templates/architecture/ADR.template.md` for hard-to-reverse, protected-boundary, or cross-feature technical decisions that deserve an independent decision record. Agents may propose ADRs; the accountable repository owner accepts, amends, defers, or rejects them.

ADR acceptance does **not** authorize feature implementation. After owner review, synchronize accepted decisions in this direction:
...
Any contradiction between those artifacts blocks affected work until resolved.
...
Use `architecture-coverage-review` at architecture synchronization, implementation-readiness, convergence, or documentation-reconciliation checkpoints when applicable. Do not infer `Not applicable` merely because a concern is absent.
...
See `templates/architecture/README.md` for the artifact-by-artifact architecture workflow.
...
2. Install AGENTS.md and applicable policy modules.
3. Create AI_CONTEXT.md and DECISION_REGISTER.md.
4. Install templates and the selected platform skills.
5. Create/refresh ARCHITECTURE_SPINE.md from verified facts and authority.
6. Create proposed ADRs for cross-feature/hard-to-reverse decisions.
7. Owner reviews ADRs.
8. Synchronize accepted ADRs → spine → coverage → readiness → AI_CONTEXT → decision register.
9. Resolve architecture contradictions/blockers needed for the first bounded slice.
...
| 13 | Implement bounded task group | `bounded-feature-implementation` | Approved artifacts + exact authorization | Code/config/docs + TASK-CLOSURE evidence | Only authorized scope |
...
6. Create the platform Architecture Spine and required proposed ADRs; obtain owner ADR decisions and synchronize architecture artifacts.
7. Create architecture coverage/readiness files when applicable, then record their canonical paths in `AI_CONTEXT.md`.
8. Select one bounded feature; create/refine and review Feature Input before contract authoring.
9. Replace every remaining placeholder and record canonical paths and owners.
10. Do not install files under `historical/` as active authority.

## Playbooks

- `playbook/AI_Greenfield_Project_Playbook_Mobile_Generic_v2_2_Agent_Native.docx` is the latest agent-native detailed playbook. It follows the same PRD → architecture bootstrap → normalized Feature Input → contract/plan/tasks → readiness → bounded implementation → convergence flow described here.
- Earlier playbooks under `playbook/` or `historical/` are retained only as prior editions when present and are not canonical.
