...
| Mutation / Commit | Authoritative Source of Truth | Affected Consumer / Screen | Required Observable Update | Update Timing | Verification Scenario |
|---|---|---|---|---|---|
| {{MUTATION}} | {{SOURCE}} | {{CONSUMER}} | {{OBSERVABLE_UPDATE}} | {{TIMING}} | {{SCENARIO}} |
...
A successful local write is not sufficient when another currently relevant consumer can remain stale. Entry context must not be inferred from a generic save action; onboarding/setup mode and normal management mode require explicit result semantics.
