...
| Mutation | Authoritative Write | Affected Consumers | Invalidation / Observation Mechanism | Expected Timing | Stale-State Risk | Verification |
|---|---|---|---|---|---|---|
| {{MUTATION}} | {{WRITE_BOUNDARY}} | {{CONSUMERS}} | {{UPDATE_MECHANISM}} | {{TIMING}} | {{STALE_RISK}} | {{VM_ID_OR_SCENARIO}} |

For every durable mutation, trace producer → committed source of truth → resolver/read model → every affected visible consumer. Do not accept local state replacement, navigation-triggered accidental reload, or app relaunch as the only refresh mechanism unless explicitly required by authority.

### Vertical Journey Closure

Every user-committable slice must have at least one executable journey proving: `commit → durable success → affected consumer refresh → correct origin-sensitive destination`. Where a mutation intentionally does not affect a consumer, record that invariant and test it.
