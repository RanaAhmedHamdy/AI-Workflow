...
description: "Executes an explicitly authorized, dependency-valid group of iOS implementation tasks and produces task-level verification evidence. Use only when the feature contract, plan, tasks, readiness audit, and exact implementation slice are already approved."
---
...
Execute an explicitly authorized, dependency-valid group of iOS implementation tasks and produce the task-level verification and evidence required to close that group.
...
- applicable iOS readiness skills;
...
- production Swift/Objective-C source;
- tests and fixtures;
...
- task type for each selected task: `DESIGN-BOUND UI`, `NON-DESIGN UI`, or `NON-UI`.
...
For every `DESIGN-BOUND UI` task, before writing UI code:

1. resolve the exact approved visual artifact and required variant/state from the task Design Lock;
2. inspect the actual visual artifact (local preferred; deterministic approved external retrieval is acceptable);
3. read the associated screen contract/design-system rules only as supplements;
4. preserve approved composition, hierarchy, grouping, relative sizing, action placement, spacing/typography intent, and listed invariants;
5. apply only the task's documented permitted platform/accessibility/localization/RTL/responsive/owner-approved adaptations.

If the exact approved visual cannot be inspected, stop with `BLOCKED BY AUTHORITY OR SCOPE`; do not infer the screen from Markdown, artifact IDs, framework defaults, or subjective “more native” preference. Inspect each unchanged governing visual once per coherent task execution; do not repeatedly reload it after every edit.

Confirm:

- actual target/package/module ownership;
...
- accepted target/package/module ownership;
- dependency direction and composition root;
- presentation-state ownership and actor isolation;
...
- the Design Lock for every design-bound screen;
...
- redesign, reorder, regroup, or replace approved screen composition merely because a framework-default or “more native” arrangement is easier.
...
Examples may include focused unit/state tests, repository or migration tests, UI tests, simulator/device scenarios, VoiceOver checks, lifecycle/relaunch checks, or platform configuration inspection when the task requires them.
...
- generated or project-file churn;
...
## Evidence Timing Execution Rule

Before running any named verification, read its timing classification from the plan/tasks.

- Execute `TASK-CLOSURE` evidence for the selected task group.
- Do not execute `FEATURE-CONVERGENCE` or `FINAL-READINESS` evidence during normal bounded implementation unless the selected task explicitly owns that gate or the current mutation directly creates an immediate protected-boundary risk.
- Reuse valid current artifacts; do not rerun solely to create separate `VM-*` filenames.
- For long-running commands, prefer completion/exit status plus compact summary and artifact path. Do not repeatedly ingest full logs.
- A task still cannot close without its required `TASK-CLOSURE` evidence and vertical journey proof where applicable.
