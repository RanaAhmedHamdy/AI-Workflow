...
`.templates/mobile/android/IMPLEMENTATION_TASKS_TEMPLATE.md`
...
Every route/screen task must classify `DESIGN-BOUND UI`, `NON-DESIGN UI`, or `NON-UI` and state, as applicable:

- screen contract and exact approved visual artifact path/locator plus required variant/state;
- a concise Design Lock for design-bound UI: binding composition/hierarchy/grouping/relative sizing/action placement plus only documented permitted adaptations;
- explicit pre-code requirement to inspect the actual approved visual; artifact IDs/Markdown alone are insufficient;
...
- design authority or an inspectable exact approved visual is missing for a design-bound planned screen;
