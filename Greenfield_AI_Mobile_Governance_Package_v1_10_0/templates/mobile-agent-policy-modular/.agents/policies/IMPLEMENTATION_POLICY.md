...
- classify each UI task as design-bound or not design-bound;
- for design-bound UI, inspect the exact approved visual artifact/variant before writing UI code; artifact IDs, screen contracts, and design-system Markdown do not replace visual inspection;
- preserve the task Design Lock (composition, hierarchy, grouping, relative sizing, action placement, spacing/typography intent) except for documented platform-required, accessibility, localization/RTL, responsive, or owner-approved adaptations;
- block if the exact approved visual cannot be inspected instead of redesigning from framework defaults or subjective “more native” preference;
...
Inspect each unchanged governing visual once per coherent task execution; do not repeatedly reload it or require a screenshot-diff review by default. Post-hoc visual comparison is reserved for explicit risk, regression, owner concern, or final-readiness requirements.
...
## 11. Evidence execution economy

- During bounded implementation, run only `TASK-CLOSURE` evidence plus any immediate protected-boundary check made necessary by the current change.
- Defer `FEATURE-CONVERGENCE` and `FINAL-READINESS` evidence to their owning gates.
- Reuse current valid evidence across multiple verification rows when coverage is real.
- Prefer coherent vertical tasks over ceremonial task splitting; every concern needs ownership, not necessarily its own task.
- Prefer deterministic summaries and artifact references over repeated full-log polling.
- Use manual final validation for subjective/device-only properties when it gives equal or better confidence at lower automation cost.

## 12. Documentation convergence

During a bounded task, `documentation-impact-assessment` should identify only whether documentation is affected and name the affected canonical files. Full documentation consistency and architecture-coverage reconciliation belong at feature convergence unless the task changes an authority document that must be synchronized immediately. Avoid running multiple exhaustive documentation reviews over the same unchanged material.
