...
## 2. Optional design authority setup

When completed external designs exist, preserve the exact artifacts, record explicit owner approval, and map approved screen/state variants to stable artifact IDs/paths. Imported files are not authoritative merely because they exist. Local approved artifacts are preferred when practical; design-tool MCP/connectors are optional and external revisions do not silently supersede repository authority.

## 3. Architecture bootstrap

Before bounded feature intake for a non-trivial greenfield app:

- gather repository facts as evidence, not automatic decisions;
- extract architecture-driving constraints from approved product/design/governance authority;
- seed the Draft Architecture Spine;
- propose ADRs only for significant decisions;
- require explicit owner authority for ADR status promotion;
- use provisional acceptance plus bounded feasibility evidence where a decision cannot yet be proven;
- synchronize accepted decisions and run architecture coverage in bootstrap mode;
- reach `READY FOR BOUNDED FEATURE INTAKE` or record an explicit blocker.

Do not claim the feature Implementation Readiness Gate passes at this phase; Feature Contract, Plan, and Tasks do not yet exist.
...
## 8. Pre-implementation readiness
...
- for design-bound UI, exact approved visuals are inspectable and tasks carry a Design Lock with permitted adaptations;
...
## 10. Bounded feature implementation

Execute dependency-ordered tasks one explicitly authorized coherent slice at a time using `bounded-feature-implementation` or an equivalent production-mutation procedure.

The implementation procedure owns production mutation, task-required preflights, focused verification, task-level evidence capture, and bounded diff hygiene. It does not replace the independent whole-feature convergence review.

During implementation:

- bind work to exact authorized task IDs and stop at that boundary;
- inspect existing repository boundaries before editing;
- for every design-bound UI task, inspect the exact approved visual artifact before UI coding and preserve its Design Lock; block if the visual is unavailable instead of substituting Markdown-only interpretation;
- preserve contract and plan boundaries;
- stop on scope, architecture, or protected-boundary drift;
- do not add later-feature placeholders or unrelated refactors;
- do not self-authorize amendments;
- run the exact tests and evidence procedures named by the selected tasks;
- fix defects only when the repair remains inside the same authorized task scope and approved plan;
- attach actual evidence as tasks close;
- leave unsupported runtime work open;
- do not mark completion from file existence or compilation alone.

Task-local checks must not be expanded into a repeated whole-feature convergence audit. Broad cross-task traceability, complete architecture/design conformance, full production-composition review, and `READINESS_REPORT.md` creation remain responsibilities of feature implementation convergence.

## 11. Feature implementation convergence
...
## 13. Independent readiness review
