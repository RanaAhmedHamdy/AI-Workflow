...
## 5. Design-bound UI fidelity at source

For a UI task governed by an approved visual artifact:

- the task must identify the exact approved artifact/variant;
- the implementation procedure must inspect the actual visual artifact **before writing UI code**; artifact IDs or Markdown descriptions alone are insufficient visual input;
- preserve approved composition, hierarchy, grouping, relative sizing, action placement, and spacing/typography intent unless a documented platform-required, accessibility, localization/RTL, responsive, or owner-approved adaptation applies;
- “more native,” framework defaults, or implementation convenience do not authorize material redesign;
- if the exact approved visual cannot be inspected, block rather than improvise.

Efficiency: inspect each unchanged governing visual once per coherent task execution and reuse that understanding. Do not require a separate screenshot-diff/fidelity-review loop for every screen by default; reserve post-hoc visual review for explicit risk, regression, owner concern, or final-readiness requirements.

## 6. Evidence economy without weaker proof
