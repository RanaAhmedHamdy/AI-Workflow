...
This package refactors one very large repository policy into a small authoritative root file plus routed policy modules. Architecture/design workflows remain in their dedicated template READMEs; feature-delivery invariants are shared by both platforms.
...
| Architecture bootstrap/status, contract, plan, task, approval, acceptance | `.agents/policies/GOVERNANCE_LIFECYCLE.md` |
...
- `.agents/policies/FEATURE_DELIVERY_INVARIANTS.md` centralizes complexity, journey coherence, mutation propagation, design-bound pre-code visual inspection, evidence timing/reuse, and representative/manual runtime rules used by both platforms.
