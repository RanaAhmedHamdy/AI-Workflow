...
## Project bootstrap before feature delivery

Approved product authority  
→ optional explicit approval of completed external design artifacts  
→ architecture bootstrap (repository facts as evidence, Draft Spine, ADR proposals)  
→ explicit owner ADR decisions / provisional evidence where required  
→ architecture synchronization + coverage/consistency  
→ `READY FOR BOUNDED FEATURE INTAKE`.

The feature Implementation Readiness Gate is later and cannot PASS before Feature Contract, Plan, and Tasks exist.
...
- For design-bound UI, inspect the exact approved visual artifact before writing UI code and preserve its Design Lock; if the visual is unavailable, block rather than infer the screen from Markdown or framework conventions.
...
- architecture bootstrap and ADR lifecycle governance;
