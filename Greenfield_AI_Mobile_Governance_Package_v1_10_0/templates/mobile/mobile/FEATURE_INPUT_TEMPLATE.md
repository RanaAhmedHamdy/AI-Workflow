...
For each required screen or reusable surface, cite the governing approved screen contract/design artifact when available. Prefer a stable local artifact path or exact approved revision/locator. Note material modes/states that share the same visual surface but have different product behavior. Design-tool MCP/connectors are optional; repository-approved design authority controls which revision is current.
