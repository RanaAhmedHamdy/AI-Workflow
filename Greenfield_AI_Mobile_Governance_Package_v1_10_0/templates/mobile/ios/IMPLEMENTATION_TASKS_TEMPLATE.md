...
**Design Lock — required for design-bound UI tasks**
- Design binding: {{DESIGN_BOUND_OR_NOT_DESIGN_BOUND}}
- Exact approved visual artifact(s)/path(s): {{EXACT_APPROVED_VISUALS}}
- Required variant/state: {{VARIANT_STATE}}
- Preserve: {{COMPOSITION_HIERARCHY_GROUPING_RELATIVE_SIZING_ACTION_PLACEMENT_SPACING_TYPOGRAPHY_INTENT}}
- Permitted adaptations only: {{DOCUMENTED_PLATFORM_ACCESSIBILITY_LOCALIZATION_RTL_RESPONSIVE_OR_OWNER_APPROVED_ADAPTATIONS}}
- Pre-code inspection: REQUIRED / N/A

For `DESIGN-BOUND`, the implementation agent must inspect the actual approved visual before writing UI code. Artifact IDs, Screen Contracts, and Design System Markdown do not replace visual inspection. If the visual cannot be inspected, stop rather than improvise.
...
- [ ] If design-bound, the exact approved visual was inspected before UI implementation and the Design Lock was preserved; no unapproved redesign was introduced.
...
- [ ] Exact screen contract plus exact approved visual artifact path/locator and required variant/state.
- [ ] Design binding is classified; design-bound tasks carry a Design Lock and require pre-code visual inspection.
