...
- Review mode: BOOTSTRAP / FEATURE-READINESS / CONVERGENCE
- Reviewed scope/revision:

In `BOOTSTRAP` mode, honest `Implementation: Not started` and `Evidence: None` are valid when the required decision is established and no intended first-slice blocker is hidden. Bootstrap readiness is not implementation readiness.
