...
`templates/mobile/FEATURE_INPUT_TEMPLATE.md` now captures the product journey before contract authoring: invocation origin and mode, exact success/cancel/failure destinations, durable mutations and affected consumers, critical end-to-end journeys, lifecycle/re-entry expectations, and complexity/risk signals. The same visual screen may be reused from multiple origins, but each origin keeps its own behavioral return semantics.
